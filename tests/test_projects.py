...
def test_post_new_project(new_project_data):
    ...
    assert response.json()['iteration_length'] == \
           new_project_data['iteration_length']
    assert response.json()['week_start_day'] == \
           new_project_data['week_start_day']
...
def test_put_update_project(update_project_data, fixture_project):
    ...
    response = my_request_controller. \
        send_request('PUT',
                     f"/projects/{test_id}",
                     payload=update_project_data)

    assert HTTPStatus.OK.value is response.status_code
    assert response.json()['name'] == \
           update_project_data['name']
    assert response.json()['week_start_day'] == \
           update_project_data['week_start_day']


def test_delete_project(fixture_project):
    _, test_id = fixture_project
    response = my_request_controller. \
        send_request('DELETE', f'/projects/{test_id}')
